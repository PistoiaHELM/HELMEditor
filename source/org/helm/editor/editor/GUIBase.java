 ...
 * <p/>
 * To avoid problems with "calls to overriden method in constructor", do not
 * initializeIOHandler the demo within the constructor of the subclass, use the
 * method {@link #initializeIOHandler()} instead.
 */
public abstract class GUIBase {
	...
	protected JPanel viewPanel; // a panel that contains the view and the
								// sequence view panel
	protected SequenceViewPanes tabbedSequenceViewPanel;
	protected Graph2DViewActions actions;
	protected ActionMap amap;
	protected GraphManager graphManager; // a hyper graph is a clustering of the
											// graph being displayed. each
											// hypernode
	...
	protected GUIBase() {
		...
		viewSplitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT, view,
				tabbedSequenceViewPanel);
	...
	public void loadGraph(Class aClass, String resourceString) {
		String fqResourceName = aClass.getPackage().getName().replace('.', '/')
				+ '/' + resourceString;

		URL resource = aClass.getResource(resourceString);
		if (resource == null) {
			String message = "Resource \"" + fqResourceName
		...
		try {
			...
			String message = "Unexpected error while loading resource \""
					+ fqResourceName + "\" due to " + e.getMessage();
			D.showError(message);
	...
	/**
	 * Creates an application frame for this demo and displays it. The class
	 * name is the title of the displayed frame.
	 */
	...
	public final void start(String title) {
		...
		frame.setIconImage(IconGenerator
	...
	protected JToolBar createToolBar() {
		...
		if (isDeletionEnabled()) {
			toolBar.add(new AbstractAction("Clear") {
				...
				{
					URL imageURL = GUIBase.class
							.getResource("resource/New16.gif");
					if (imageURL != null) {
		 ...
		 * toolBar.add(pasteAction); toolBar.addSeparator();
		 */
		...
		magnifierButton = new JToggleButton(new ImageIcon(
				GUIBase.class.getResource("resource/Magnifier16.gif")));
	...
	protected JMenuBar createMenuBar() {
		...
		menu.add(new SaveAsImage("Save sequence view image as ...",
				tabbedSequenceViewPanel.getSequenceView()));
		menu.addSeparator();
	...
	public static String generateApplicationTitle() {
		...
		String title = null;
		title = resourceBundle.getString("Application.name") + " "
				+ resourceBundle.getString("Application.id") + "v"
				+ resourceBundle.getString("Application.version");
		return title;
	}
	...
	/**
	 * Action that saves the current subset of the graph to a file in YGF
	 * format.
	 */
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (chooser.showSaveDialog(contentPane) == JFileChooser.APPROVE_OPTION) {
				...
				if (name.endsWith(".ygf")) {
					...
					try {
						DataProvider dp = Selections
								.createSelectionDataProvider(view.getGraph2D());
					...
					try {
						DataProvider dp = Selections
								.createSelectionDataProvider(view.getGraph2D());
						graphMLIOHandler.writeSubset(view.getGraph2D(), dp,
								name);
					} catch (IOException ioe) {
						D.show(ioe);
					}
	...
	class GraphDeepCopyFactory extends Graph2DCopyFactory {
		...
		void copyNodeData(final Node source, final Node target) {
			...
			MonomerInfo monomerInfo = (MonomerInfo) monomerInfoNodeMap
					.get(source);
			MonomerInfo newMonomerInfo = new MonomerInfo(
					monomerInfo.getPolymerType(), monomerInfo.getMonomerID());
		...
		void copyEdgeData(final Edge source, final Edge target) {
			...
			MonomerInfo sourceMonomerInfo = (MonomerInfo) monomerInfoNodeMap
					.get(sourceNode);
			Attachment sourceAtt = sourceMonomerInfo.getAttachment(edgeInfo
					.getSourceNodeAttachment().getLabel());

			MonomerInfo targetMonomerInfo = (MonomerInfo) monomerInfoNodeMap
					.get(targetNode);
			Attachment targetAtt = targetMonomerInfo.getAttachment(edgeInfo
	...
	protected class Zoom extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			view.setWorldRect(box.x - 20, box.y - 20, box.width + 40,
					box.height + 40);
	...
	protected class FitContent extends AbstractAction {
		...
		public FitContent() {
			super("Fit Content");
			URL imageURL = GUIBase.class
					.getResource("resource/FitContent16.gif");
			if (imageURL != null) {
				this.putValue(Action.SMALL_ICON, new ImageIcon(imageURL));
			}
	...
	protected class SequenceGraph2DClipboard extends Graph2DClipboard {
		...
		public NodeList pasteFromClipBoard(Graph targetGraph) {
			try {
				...
				Node node = null;
				NodeMap nodeMap = (NodeMap) targetGraph
						.getDataProvider(NodeMapKeys.MONOMER_REF);
				...
				for (int i = 0; i < nodeList.size(); i++) {
					...
					if (node.inDegree() == 0
							|| (monomerInfo != null && monomerInfo
									.getPolymerType().equalsIgnoreCase(
				...
				JOptionPane.showMessageDialog(null, ex.getMessage(),
						"Invalid Monomer!", JOptionPane.WARNING_MESSAGE);
				Logger.getLogger(MacromoleculeEditor.class.getName()).log(
						Level.SEVERE, ex.getMessage());
			} catch (JDOMException ex) {
				JOptionPane.showMessageDialog(null, ex.getMessage(),
						"JDOMException!", JOptionPane.WARNING_MESSAGE);
				Logger.getLogger(MacromoleculeEditor.class.getName()).log(
						Level.SEVERE, ex.getMessage());
			}
			return null;
